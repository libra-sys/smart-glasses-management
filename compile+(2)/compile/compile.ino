...
// ===== WiFi / Server =====
const char* WIFI_SSID   = "cyxy";
const char* WIFI_PASS   = "88888888";
const char* SERVER_HOST = "10.0.143.163";
...
framesize_t g_frame_size = FRAMESIZE_QVGA;  // 降低到QVGA(320x240)以提高帧率
#define JPEG_QUALITY  25  // 提高压缩率(数值越大压缩越强,文件越小)
#define FB_COUNT      2
...
const char* UDP_HOST  = "10.0.143.163";
...
void initBLE() {
  ...
  uint8_t vol = current_volume;
  pCharVolume->setValue(&vol, 1);
  ...
  uint8_t bat = current_battery;
  pCharBattery->setValue(&bat, 1);
  ...
  const char* status = "{\"wifi\":true,\"camera\":true,\"audio\":true}";
...
void updateBLEVolume(uint8_t vol) {
  if (pCharVolume && ble_device_connected) {
    ...
    uint8_t v = current_volume;
    pCharVolume->setValue(&v, 1);
    pCharVolume->notify();
  }
}
...
bool init_camera() {
  ...
  config.xclk_freq_hz = 16000000;  // OV3660降低XCLK频率到16MHz以提高稳定性
